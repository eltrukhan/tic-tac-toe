...
CalculateCenter[draglist_, scale_] := ({dx,dy} = draglist[[-1]]-draglist[[1]];center-={{dx,dx}, {dy,dy}}/scale)
...
DetermineVictory[] := (res = {ListConvolve[{mask1},gamefield],ListConvolve[Transpose[{mask1}],gamefield],ListConvolve[mask2,gamefield],ListConvolve[Reverse[mask2], gamefield]};
If[MemberQ[res, 5,3], victoryPos=First@Position[res,5];KrestikiWin[],
 If[MemberQ[res,-5,3], victoryPos=First@Position[res,-5];NolikiWin[]]])

ClearAll[KrestikiWin];
KrestikiWin[] := (Print["\:041a\:0440\:0435\:0441\:0442\:0438\:043a\:0438 \:043f\:043e\:0431\:0435\:0434\:0438\:043b\:0438!"]; krest++;
 CalculateVictoryPoints[];isOver = True;);

ClearAll[NolikiWin];
NolikiWin[] := (Print["\:041d\:043e\:043b\:0438\:043a\:0438 \:043f\:043e\:0431\:0435\:0434\:0438\:043b\:0438!"];noliki++; CalculateVictoryPoints[];isOver = True;)
...
Piecewise[{
{victorypoint2 =victorypoint1+{4,0},victoryPos[[1]]==1},
{victorypoint2 =victorypoint1+{0,-4},victoryPos[[1]]==2},
{victorypoint2 =victorypoint1+{4,-4},victoryPos[[1]]==3},
{victorypoint1=victorypoint1+{0,-4};victorypoint2 =victorypoint1+{4,4},victoryPos[[1]]==4}}];)
...
FirstMoveNolik[]:=(currentMove=-1;DetermineVictory[difficulty];a="\:0412\:044b \:0438\:0433\:0440\:0430\:0435\:0442\:0435 \:0437\:0430 \:043d\:043e\:043b\:0438\:043a");
...
ClearAll[ClearField];
ClearField[] := (gamefield = ConstantArray[0,{5,5}]; kr = {}; nol = {}; cellas = <||>; currentMove = 1;isOver=False;a =Column[{"\:0412\:044b \:0438\:0433\:0440\:0430\:0435\:0442\:0435 \:0437\:0430 ",
...
ClearAll[Init];
Init[] := (gamefield = ConstantArray[0, {5,5}];kr = {};nol = {};cellas = <||>;
isOver = False;
difficulty=0;
currentMove = 1;
hintkr={};
...
ClearAll[Game];
Game[] := (Init[];Row[{EventHandler[Dynamic@Graphics[{LightGray,HighlightRectangle[MousePosition["Graphics"]],Darker[Gray], Thick,DrawLines[minlimx,maxlimx,minlimy, maxlimy] ,AbsoluteThickness[2],Green,krestik/@hintkr,nolik/@hintnol,Lighter[Blue,.4],krestik/@kr,Lighter[Orange,.4], nolik/@nol,Red,AbsoluteThickness[4],Opacity[.5],DrawVictoryLine[]},PlotRange-> (center+scale*width), Background-> White,ImageSize->500], { "MouseClicked":>(mousepos = Round[MousePosition["Graphics"]];MakeOneMoveAndBot[mousepos,difficulty]),"MouseDragged" :> (UpdateDraglist[MousePosition["Graphics"]];CalculateCenter[draglist, scale] ),
"MouseUp" :> (draglist={};
...
DetermineVictory[difficulty_] := (res = {ListConvolve[{mask1},gamefield],ListConvolve[Transpose[{mask1}],gamefield],ListConvolve[mask2,gamefield],ListConvolve[Reverse[mask2], gamefield]}; If[MemberQ[res, 5(-currentMove),3], victoryPos=First@Position[res,5(-currentMove)];If[currentMove == 1, NolikiWin[],KrestikiWin[]], Bot[difficulty]; DetermineVictory[]])
...
ConstructFreeLine[point_, direction_]:= ((point + # direction)&/@Range[4])
...
SlidingLinePoints[point_, direction_, player_]:= (Max[Count[#,player]&/@Partition[line,  5, 1]])

ClearAll[TotalLinePoints];
TotalLinePoints[point_,direction_, player_] := (
line = Join[segment1 =ConstructLineWithSymbols[point, -direction, player], {player},segment2 = ConstructLineWithSymbols[point, direction, player] ];If[Length[line] >=5,Max[90*ConsecutiveLinePoints[point,direction, player], 100*SlidingLinePoints[point,direction, player]],0])
...
ClearAll[Hint];
Hint[]:=(hint=HardBotMove[];If[currentMove==1,hintkr={hint},hintnol={hint}]);
...
MediumBotMove[] :=
If[cellas == <||>, {0,0},(cells =FindAvailableCells[];UpdateFieldMedium[#,0]& /@ cells; matcells = FromGraphicsToMatrix/@cells;extendedGamefield = ArrayPad[gamefield,5];f1 = (#==-currentMove)&;
f2 = (#==currentMove)&;
res1 = {LengthWhile[extendedGamefield[[#[[1]], #[[2]]-1;;#[[2]]-4;;-1]], f1]+LengthWhile[extendedGamefield[[#[[1]], #[[2]]+1;;#[[2]]+4]], f1],LengthWhile[extendedGamefield[[#[[1]]-1;;#[[1]]-4;;-1, #[[2]]]], f1]+LengthWhile[extendedGamefield[[#[[1]]+1;;#[[1]]+4, #[[2]]]], f1],LengthWhile[Reverse[Diagonal[extendedGamefield[[#[[1]]-4;;#[[1]]-1, #[[2]]-4;;#[[2]]-1]]]], f1]+LengthWhile[Diagonal[extendedGamefield[[#[[1]]+1;;#[[1]]+4, #[[2]]+1;;#[[2]]+4]]], f1],LengthWhile[Reverse[Diagonal[Reverse[extendedGamefield[[#[[1]]+1;;#[[1]]+4, #[[2]]-4;;#[[2]]-1]]]]], f1]+LengthWhile[Diagonal[Reverse[extendedGamefield[[#[[1]]-4;;#[[1]]-1, #[[2]]+1;;#[[2]]+4]]]], f1]}& /@ (matcells+5);
res2 = {LengthWhile[extendedGamefield[[#[[1]], #[[2]]-1;;#[[2]]-4;;-1]], f2]+LengthWhile[extendedGamefield[[#[[1]], #[[2]]+1;;#[[2]]+4]], f2],LengthWhile[extendedGamefield[[#[[1]]-1;;#[[1]]-4;;-1, #[[2]]]], f2]+LengthWhile[extendedGamefield[[#[[1]]+1;;#[[1]]+4, #[[2]]]], f2],LengthWhile[Reverse[Diagonal[extendedGamefield[[#[[1]]-4;;#[[1]]-1, #[[2]]-4;;#[[2]]-1]]]], f2]+LengthWhile[Diagonal[extendedGamefield[[#[[1]]+1;;#[[1]]+4, #[[2]]+1;;#[[2]]+4]]], f2],LengthWhile[Reverse[Diagonal[Reverse[extendedGamefield[[#[[1]]+1;;#[[1]]+4, #[[2]]-4;;#[[2]]-1]]]]], f2]+LengthWhile[Diagonal[Reverse[extendedGamefield[[#[[1]]-4;;#[[1]]-1, #[[2]]+1;;#[[2]]+4]]]], f2]}& /@ (matcells+5);
